import { PluginManifest, BTCPayPermission } from '@/types/plugin';
...
export class PluginManifestValidator {
  private readonly requiredFields = ['id', 'name', 'version', 'description', 'author'];
  ...
  validate(manifestData: any): ManifestValidationResult {
    ...
    if (manifestData.name) {
      ...
      if (!/^[\w\s\-\.]+$/.test(manifestData.name)) {
        warnings.push({
          ...
          recommendation: 'Use only letters, numbers, spaces, hyphens, and periods'
        });
      }
    }
    ...
    if (manifestData.description) {
      ...
      if (this.containsSuspiciousContent(manifestData.description)) {
        warnings.push({
          ...
          recommendation: 'Review description for misleading or malicious content'
        });
      }
    }
    ...
    if (manifestData.license) {
      const validLicenses = ['MIT', 'Apache-2.0', 'GPL-3.0', 'BSD-3-Clause', 'ISC', 'MPL-2.0'];
      ...
      if (typeof manifestData.routes !== 'object') {
        ...
        for (const [key, value] of Object.entries(manifestData.routes)) {
          if (typeof value !== 'string') {
            ...
            errors.push({
              field: `routes.${key}`,
              message: 'Route must start with / and contain only valid URL characters',
              value
            });
          }
        }
      ...
      if (!Array.isArray(manifestData.requiredPermissions)) {
        ...
        for (let i = 0; i < manifestData.requiredPermissions.length; i++) {
          ...
          if (!perm.permission) {
            ...
            if (this.dangerousPermissions.includes(perm.permission)) {
              warnings.push({
                ...
                recommendation: 'This permission grants significant access. Ensure it is necessary.'
              });
            }
          }
          ...
          if (!perm.description) {
            warnings.push({
              ...
              recommendation: 'Add a description to explain why this permission is needed'
            });
          }
        ...
        if (manifestData.requiredPermissions.length > 10) {
          warnings.push({
            ...
            recommendation: 'Consider reducing permissions to the minimum necessary'
          });
        }
      }
    }
    ...
    if (manifestData.dependencies) {
      if (typeof manifestData.dependencies !== 'object') {
        ...
        for (const [pkg, version] of Object.entries(manifestData.dependencies)) {
    ...
    const knownFields = [
      ...this.requiredFields,
      'homepage', 'license', 'main', 'icon', 'routes',
      'requiredPermissions', 'dependencies', 'isPaid',
      'price', 'category', 'tags', 'screenshots', 'minAppVersion'
    ];
    ...
    return {
      ...
      errors,
      warnings,
      manifest: errors.length === 0 ? manifestData as PluginManifest : undefined
    };
  }
  ...
  private validateVersion(version: string): boolean {
    return /^\d+\.\d+\.\d+(-[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*)?(\+[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*)?$/.test(version);
  }

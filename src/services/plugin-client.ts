...
export async function getAvailablePluginsClient(): Promise<AvailablePlugin[]> {
  ...
  return [
    ...
    },
    {
      ...
      version: "1.0.0",
      description:
        "Analyze your BTCPay Server transactions and generate financial reports",
      author: "BTCPayServer Companion",
      icon: "trending-up",
    ...
    {
      id: "payment-analytics-template",
      name: "Payment Analytics (Template)",
      version: "1.0.0",
      description:
        "A template plugin demonstrating BTCPayServer Companion plugin development best practices and Greenfield API integration",
      author: "BTCPayServer Companion",
      icon: "trending-up",
      route: "/apps/payment-analytics-template",
      settingsRoute: "/apps/payment-analytics-template/settings",
      routes: {
        main: "/apps/payment-analytics-template",
        settings: "/apps/payment-analytics-template/settings",
      },
      isPaid: false,
      category: "Template",
      tags: ["template", "example", "analytics", "payments"],
    },
  ];
}

export async function removePluginFolderClient(
  _pluginId: string,
...
export async function getPluginManifestClient(
  pluginId: string,
): Promise<AvailablePlugin | null> {

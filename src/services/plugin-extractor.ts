  ...
  async extractPlugin(
    ...
    try {
      ...
      return {
        success: false,
        message:
          error instanceof Error ? error.message : "Failed to extract plugin",
      };
    }
  }
    ...
    try {
      ...
      entries.forEach((entry) => {
        console.log(
          " -",
          entry.entryName,
          entry.isDirectory ? "(directory)" : `(${entry.header.size} bytes)`,
        );
      });
    ...
    try {
      ...
      if (!/^[a-z0-9-]+$/.test(manifest.id)) {
        return {
          success: false,
          message:
            "Invalid plugin ID: must contain only lowercase letters, numbers, and hyphens",
        };
      }

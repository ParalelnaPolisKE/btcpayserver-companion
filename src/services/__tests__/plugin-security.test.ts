...
describe("Plugin Security Tests", () => {
  ...
  describe("Malicious Code Detection", () => {
    it("should detect eval() usage", async () => {
      ...
      expect(
        result.issues.some(
          (i) => i.message.includes("eval") && i.severity === "critical",
        ),
      ).toBe(true);
      expect(
        result.issues.some(
          (i) =>
            i.message.includes("Environment variable") &&
            i.severity === "critical",
        ),
      ).toBe(true);
    });
    ...
    it("should detect attempts to access file system", async () => {
      ...
      expect(
        result.issues.some(
          (i) => i.message.includes("File system") && i.severity === "critical",
        ),
      ).toBe(true);
    });
    ...
    it("should detect XSS attempts", async () => {
      ...
      expect(
        result.issues.some(
          (i) => i.message.includes("innerHTML") && i.severity === "high",
        ),
      ).toBe(true);
      expect(
        result.issues.some(
    ...
    it("should detect crypto mining code", async () => {
      ...
      expect(
        result.issues.some(
          (i) => i.message.includes("WebAssembly") && i.severity === "high",
        ),
      ).toBe(true);
    });
    ...
    it("should detect obfuscated code", async () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.issues.some((i) => i.type === "obfuscation")).toBe(true);
      expect(
        result.issues.some((i) => i.message.includes("fromCharCode")),
      ).toBe(true);
    });
    ...
    it("should detect attempts to access localStorage and IndexedDB", async () => {
      ...
      expect(
        result.issues.some(
          (i) => i.message.includes("LocalStorage") && i.severity === "high",
        ),
      ).toBe(true);
      expect(
        result.issues.some(
          (i) => i.message.includes("IndexedDB") && i.severity === "high",
        ),
      ).toBe(true);
    });
    ...
    it("should detect suspicious network requests", async () => {
      ...
      expect(
        result.issues.some(
          (i) => i.message.includes("WebSocket") && i.severity === "high",
        ),
      ).toBe(true);
      expect(
        result.issues.some(
  ...
  describe("Manifest Validation", () => {
    ...
    it("should reject manifest with path traversal in main", () => {
      ...
      expect(
        result.errors.some(
          (e) => e.field === "main" && e.message.includes("valid file path"),
        ),
      ).toBe(true);
    });
    ...
    it("should warn about suspicious content in description", () => {
      const manifest = {
        ...
        version: "1.0.0",
        description:
          "This plugin will hack your BTCPay server and bypass all security!",
        author: "Hacker",
      };
      ...
      expect(
        result.warnings.some(
          (w) => w.field === "description" && w.message.includes("suspicious"),
        ),
      ).toBe(true);
    });
    ...
    it("should warn about suspicious dependencies", () => {
      ...
      expect(
        result.warnings.filter(
          (w) =>
            w.field.includes("dependencies") &&
            w.message.includes("suspicious"),
        ).length,
      ).toBeGreaterThan(0);
    });
  });
  ...
  describe("Suspicious File Detection", () => {
    it("should detect executable files", async () => {
      await fs.writeFile(
        path.join(tempDir, "malware.exe"),
        Buffer.from([0x4d, 0x5a]),
      ); // MZ header
      await fs.writeFile(
        path.join(tempDir, "backdoor.dll"),
        Buffer.from([0x4d, 0x5a]),
      );
      await fs.writeFile(
        path.join(tempDir, "manifest.json"),
      ...
      expect(
        result.issues.some(
          (i) => i.message.includes("Executable") && i.severity === "critical",
        ),
      ).toBe(true);
      expect(
        result.issues.some(
          (i) => i.message.includes("DLL") && i.severity === "critical",
        ),
      ).toBe(true);
    });
    ...
    it("should detect shell scripts", async () => {
      await fs.writeFile(
        path.join(tempDir, "exploit.sh"),
        "#!/bin/bash\nrm -rf /",
      );
      await fs.writeFile(
        path.join(tempDir, "backdoor.ps1"),
      ...
      expect(
        result.issues.some(
          (i) => i.message.includes("Shell script") && i.severity === "high",
        ),
      ).toBe(true);
      expect(
        result.issues.some(
          (i) => i.message.includes("PowerShell") && i.severity === "high",
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Security Score Calculation", () => {
    it("should calculate appropriate security scores", async () => {
      ...
      // Suspicious plugin
      await fs.writeFile(
        path.join(tempDir, "suspicious.js"),
        `

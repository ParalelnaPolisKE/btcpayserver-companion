...
export async function loadAvailablePlugins(): Promise<PluginManifest[]> {
  ...
  try {
    ...
    try {
      const installedPlugins = await loadPluginsFromDirectory(INSTALLED_PLUGINS_DIR);
 ...
 */
async function loadPluginsFromDirectory(dir: string): Promise<PluginManifest[]> {
  const plugins: PluginManifest[] = [];
...
export async function loadPlugin(pluginId: string): Promise<LoadedPlugin | null> {
  ...
  try {
    ...
    const componentPath = path.join(pluginPath, manifest.main || 'index.tsx');
 ...
 */
export async function installPluginFromZip(zipBuffer: Buffer): Promise<PluginManifest> {
 ...
 */
export function getPluginComponentPath(pluginId: string, isInstalled: boolean = false): string {

      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          plugins.push({
            ...
            settingsRoute: manifest.routes?.settings
              ? `/apps/${manifest.id}/settings`
              : undefined,
          ...
          console.error(
            `Failed to load plugin manifest from ${pluginPath}:`,
            error,
          );
        }
      }
...
export async function removePluginFolder(
  pluginId: string,
...
export async function uploadPluginServerAction(
  formData: FormData,
): Promise<{ success: boolean; message: string; plugin?: AvailablePlugin }> {
      ...
      return {
        ...
        plugin: result.manifest
          ? {
              ...
              settingsRoute: result.manifest.routes?.settings
                ? `/apps/${result.manifest.id}/settings`
                : undefined,

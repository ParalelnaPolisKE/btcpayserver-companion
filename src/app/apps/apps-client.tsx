...
import { Settings, Package, CheckCircle, Trash2, LucideIcon } from 'lucide-react';
...
export default function AppsClient() {
  const { installedPlugins, installPlugin, uninstallPlugin, togglePlugin, isLoading, refreshPlugins } = usePlugins();
  const [availablePlugins, setAvailablePlugins] = useState<AvailablePlugin[]>([]);
  const [pluginsLoading, setPluginsLoading] = useState(true);
  const [uploadDialogOpen, setUploadDialogOpen] = useState(false);
  const [removeConfirmOpen, setRemoveConfirmOpen] = useState(false);
  const [pluginToRemove, setPluginToRemove] = useState<{ id: string; name: string } | null>(null);
  ...
  const handleToggle = async (pluginId: string, enabled: boolean, pluginName: string) => {
    try {
      await togglePlugin(pluginId, enabled);
      toast.success(enabled ? `${pluginName} enabled` : `${pluginName} disabled`);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto py-8">
      <div className="mb-8 flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground">
            Extend your BTCPayServer Companion with powerful apps and integrations
          </p>
        </div>
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(checked) => handleToggle(plugin.pluginId, checked, plugin.manifest.name)}
                      />
                    </div>
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {mergedPlugins.filter(plugin => !plugin.isInstalled).map((plugin) => {
                ...
                <CardFooter className="flex gap-2">
                  ...
                  {!plugin.isInstalled && !plugin.isPaid && (
                    <Button
                      size="icon"
                      variant="outline"
                      onClick={() => handleRemoveClick(plugin.id, plugin.name)}
                      title="Remove plugin"
                    >
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload a plugin as a ZIP file. The plugin must contain a manifest.json file and follow the plugin structure.
            </DialogDescription>
          </DialogHeader>
          <div className="mt-4">
            <PluginDropzone 
      ...
      <AlertDialog open={removeConfirmOpen} onOpenChange={setRemoveConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to remove "{pluginToRemove?.name}"? 
              This will permanently delete the plugin folder from your system.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

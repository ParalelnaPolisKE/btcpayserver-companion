...
export default function PluginDropzone({ onUpload, disabled }: PluginDropzoneProps) {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      clearInterval(progressInterval);
      setUploadStatus('error');
      setErrorMessage(error instanceof Error ? error.message : 'Failed to upload plugin');
      setUploadProgress(0);
    }
  }, [onUpload]);
      ...
      {uploadStatus === 'idle' && (
        ...
        >
          ...
          {isDragActive ? (
            ...
            <>
              <p className="text-lg font-medium mb-2">Drag & drop a plugin ZIP file here</p>

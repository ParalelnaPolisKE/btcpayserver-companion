...
import { Trash2, Plus, Store as StoreIcon, Filter, Edit2, Check, X, RefreshCw, XCircle } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { getAvailableStores, getStorePOSApps } from '@bps-companion/app/actions/btcpay-stores';

export function StoresManager() {
  const { stores, addStore, updateStore, removeStore } = useStores();
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [editingStoreId, setEditingStoreId] = useState<string | null>(null);
  const [availableStores, setAvailableStores] = useState<Array<{ id: string; name: string; website?: string }>>([]);
  const [loadingStores, setLoadingStores] = useState(false);
  const [posApps, setPosApps] = useState<Array<{ id: string; appName: string; title: string }>>([]);
  const [loadingPosApps, setLoadingPosApps] = useState(false);
  const [isUsingMockData, setIsUsingMockData] = useState(false);
  const [formData, setFormData] = useState({
    storeId: '',
    storeName: '',
    posFilters: [] as Array<{ id: string; name: string; filter: string; description?: string }>,
    isActive: true
  });
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.posApps.length > 0) {
          toast.success(`Found ${result.posApps.length} POS app${result.posApps.length !== 1 ? 's' : ''}`);
        }
      }
    } catch (error) {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={(open) => {
            ...
            if (!open) {
              setFormData({ storeId: '', storeName: '', posFilters: [], isActive: true });
              setCurrentFilter({ name: '', filter: '', description: '' });
              setPosApps([]);
            }
          }}>
            ...
            <DialogContent className="sm:max-w-[650px] max-h-[90vh] overflow-hidden flex flex-col">
              <DialogHeader>
                ...
                <DialogDescription>
                  Select a store from your BTCPayServer or enter details manually. The POS filter allows you to track specific item types.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4 overflow-y-auto px-1 flex-1">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Select onValueChange={handleStoreSelect} disabled={loadingStores}>
                      ...
                      <SelectContent>
                        {availableStores.map((store) => {
                          ...
                          const displayId = store.id.length > 12 
                  ...
                  {isUsingMockData && (
                    <Alert>
                      <AlertDescription className="text-xs">
                        Using sample data. Connect to BTCPayServer for real stores.
                      </AlertDescription>
                    </Alert>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {loadingPosApps && (
                    <Alert>
                      ...
                      <AlertDescription>Loading POS apps for selected store...</AlertDescription>
                    </Alert>
                  )}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {formData.posFilters.map((filter) => (
                          <Badge key={filter.id} variant="secondary" className="gap-1">
                            ...
                            <span className="text-xs opacity-70">({filter.filter})</span>
                            <button
                              type="button"
                  ...
                  <div className="space-y-2 border rounded-lg p-3">
                    ...
                    {!loadingPosApps && posApps.length > 0 && (
                      <div className="space-y-2">
                        <Select 
                          onValueChange={(value) => {
                            const app = posApps.find(a => a.appName === value);
                            if (app) {
                              setCurrentFilter({
                                name: app.title,
                                filter: app.appName,
                        ...
                        {posApps.length > 0 && (
                          <p className="text-xs text-muted-foreground">
                            Found {posApps.length} POS app{posApps.length !== 1 ? 's' : ''}
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="space-y-1">
                        <Label htmlFor="filterName" className="text-xs">Filter Name</Label>
                        <Input
                          id="filterName"
                      ...
                      <div className="space-y-1">
                        <Label htmlFor="filterValue" className="text-xs">Filter Value</Label>
                        <Input
                          id="filterValue"
                          placeholder="e.g., membership"
                          value={currentFilter.filter}
                          onChange={(e) => setCurrentFilter({ ...currentFilter, filter: e.target.value })}
                        />
                      </div>
                    </div>
                    
                    <div className="space-y-1">
                      <Label htmlFor="filterDesc" className="text-xs">Description (Optional)</Label>
                      <Input
                        id="filterDesc"
                  ...
                  <p className="text-xs text-muted-foreground">
                    Add multiple filters to track different types of transactions. You can switch between them in the dashboard.
                  </p>
                </div>
              ...
              <DialogFooter className="flex justify-between items-center pt-4 border-t">
                ...
                <Button onClick={handleAdd} disabled={!formData.storeId || !formData.storeName}>
                  Add Store
                </Button>
              </DialogFooter>
            </DialogContent>
      ...
      <CardContent>
        {stores.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm">Add your first store to start tracking financial data</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
                ...
                <TableRow key={store.storeId}>
                  {editingStoreId === store.storeId ? (
                    <>
                      ...
                      <TableCell>
                        {editFormData.posFilters && editFormData.posFilters.length > 0 ? (

    ...
    const initializeChat = async () => {
      try {
        if (!vectorServiceRef.current) {
          ...
          dataIngestionRef.current = new DataIngestionService(vectorServiceRef.current);
...
Would you like me to index your BTCPayServer data first?`,
  ...
  const indexData = useCallback(async (btcPayData: any) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: `system-${Date.now()}`,
        role: 'system',
        content: '✅ Successfully indexed BTCPayServer data. You can now ask questions about your invoices, payments, and store information.',
        timestamp: new Date(),
      }]);
    } catch (err) {
  ...
  const clearMessages = useCallback(() => {
    setMessages([{
      id: 'welcome-reset',
      role: 'assistant',
      content: '🔄 Chat cleared. How can I help you with your BTCPayServer data?',
      timestamp: new Date(),
    }]);
  }, []);

...
import {
  DEFAULT_SETTINGS,
  getCryptoChatSettings,
  saveCryptoChatSettings,
} from "../utils/store";

export default function CryptoChatSettings() {
  const [settings, setSettings] =
    useState<CryptoChatSettings>(DEFAULT_SETTINGS);
    ...
    if (settings.provider === "openai") {
      ...
      try {
        const response = await fetch(
          `${settings.ollamaUrl || "http://localhost:11434"}/api/tags`,
        );
        ...
        if (response.ok) {
          ...
          setSaveMessage(
            `✅ Connected! Available models: ${modelNames || "none"}`,
          );
        } else {
        ...
        setSaveMessage(
          "❌ Cannot reach Ollama server. Make sure it's running.",
        );
      }
    }
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setSettings({ ...settings, provider: value as any })
              }
            >
      ...
      {!settings.openaiApiKey && (
        <Alert>
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            CryptoChat requires an OpenAI API key to function. The plugin will
            use mock responses until configured.
          </AlertDescription>
        </Alert>
      )}
